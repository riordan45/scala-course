...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: String): List[(Char, Int)] = {
    w.toLowerCase.toList.groupBy(identity).map { case (char, occurrences) =>
      (char, occurrences.size)
    }.toList.sortBy(_._1)
  }
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = 
    val combinedOccurrences = s.flatMap(wordOccurrences)
    combinedOccurrences.groupBy(_._1).map { case (char, occurrences) =>
      (char, occurrences.map(_._2).sum)
    }.toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      ...
      case (char, freq) :: rest =>
        ...
        currentCombinations.map(_.sorted).distinct
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    val subtractedMap = xMap.map { case (char, freq) =>
      (char, freq - yMap.getOrElse(char, 0))
    }

    subtractedMap.filter { case (_, freq) => freq > 0 }.toList.sorted
  }
    ...
    def iter(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          rest <- iter(subtract(occurrences, combo))
        } yield word :: rest
      }
    }
