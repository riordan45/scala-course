...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    val questions = postings.filter(posting => posting.postingType == 1 && posting.parentId.isEmpty).map(posting => (posting.id, posting))
    val answers = postings.filter(posting => posting.postingType == 2 && posting.parentId.isDefined).map(posting => (posting.parentId.get, posting))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map { case (qid, qaItr) =>
        val question = qaItr.head._1
        val highScores = answerHighScore(qaItr.map(_._2).toArray)
        (question, highScores)
    }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    // Group the vectors by the closest mean
    val closestGrouped = vectors.map(p => (findClosest(p, means), p)).groupByKey()

    // Calculate the new means for each group
    val newMeansMap = closestGrouped.mapValues(averageVectors).collectAsMap()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val langCount = vs.groupBy(_._1).mapValues(_.size)
      val (langIndex, count) = langCount.maxBy(_._2)  // Find the most common language
      val langLabel: String = langs(langIndex / langSpread)  // Recover the language string using langIndex
      val langPercent: Double = count * 100.0 / vs.size  // Calculate percentage of the most common language

      val clusterSize: Int = vs.size  // Total number of questions in the cluster
      val sortedScores = vs.map(_._2).toArray.sorted  // Sort scores to find the median
