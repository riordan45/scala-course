...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("week 1 assignment")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = 
    rdd.aggregate(0)( 
      (count: Int, article: WikipediaArticle) => count + (if article.mentionsLanguage(lang) then 1 else 0),
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = 
    rdd.flatMap(article => langs.filter(article.mentionsLanguage).map(lang => (lang, article))).groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = 
    index.map{case (lang, itr) => (lang, itr.size)}.collect().toList.sortBy(-_._2)
