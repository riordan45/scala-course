...
trait Solver extends GameDef:
  ...
  def done(b: Block): Boolean = b match {
    case Block(a, b) => a == b && a.col == goal.col && a.row == goal.row
  }
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = 
    ...
    c.map((block, move) => (block, move :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter((neighbour, moves) => !explored.contains(neighbour))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      // Explore new neighbors from the current head of the lazy list
      val more = for {
        (block, history) <- initial
        next <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield next

      // Recursively call `from` with the new neighbors and updated explored set
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter((block, moves) => done(block))
  ...
  lazy val solution: List[Move] = 
    ...
    case Some((_, moves)) => moves.reverse
    case None => List()
  }
