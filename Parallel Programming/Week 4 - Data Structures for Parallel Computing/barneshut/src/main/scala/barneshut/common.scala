...
extends Quad:
  val (mass, massX, massY) = {
    ...
    val massX = bodies.map(b => b.mass * b.x).sum / totalMass
    val massY = bodies.map(b => b.mass * b.y).sum / totalMass
    (totalMass, massX, massY)
  }
  ...
  def insert(b: Body): Quad = if (size > minimumSize) {
      ...
      val fork = Fork(nw, ne, sw, se).insert(b)
  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Leaf(_, _, _, bodies) =>
        // Add force contribution of each body in this leaf node
        bodies.foreach(b => addForce(b.mass, b.x, b.y))
      case Fork(nw, ne, sw, se) =>
        // Check if the node is far enough or if we should recurse
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    val xIndex = ((b.x - boundaries.minX) / sectorSize).toInt max 0 min (sectorPrecision - 1)
    val yIndex = ((b.y - boundaries.minY) / sectorSize).toInt max 0 min (sectorPrecision - 1)
